...
import { api, send } from "./utils/api";
...
function Signup({ bootstrap, reload }) {
  ...
  const [form, setForm] = useState({ event_id: "", player_id: "", shift_id: "", primary_position_id: "", secondary_position_id: "", notes: "" });
  ...
  const submit = async (e) => {
    ...
    try {
      await send("/registrations", "POST", Object.fromEntries(Object.entries(form).map(([key, value]) => [key.endsWith("_id") && value ? Number(value) : key.endsWith("_id") ? null : value])));
  ...
  return <section className="split hero-section">
    <div className="hero-copy"><span className="eyebrow">Sua próxima partida começa aqui</span><h1>Entre em quadra.<br /><em>O Flow cuida do resto.</em></h1><p>Inscreva-se em poucos segundos, confirme sua presença e acompanhe seu time.</p><div className="rule"><b>01</b><span>Escolha o jogo</span><b>02</b><span>Confirme por e-mail</span><b>03</b><span>Veja seu time</span></div></div>
    <form className="card signup-card" onSubmit={submit}><div className="card-title"><span>Inscrição</span><small>Leva menos de 1 minuto</small></div>
      ...
      <Field label="Seu nome"><select required value={form.player_id} onChange={(e) => { const selected = players.find((item) => item.id === Number(e.target.value)); setForm({ ...form, player_id: e.target.value, primary_position_id: selected?.primary_position_id || "", secondary_position_id: selected?.secondary_position_id || "" }); }}><option value="">Selecione seu cadastro</option>{players.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></Field>
      ...
      <Button type="submit">Confirmar inscrição <span>→</span></Button>
...
function PlayersAdmin({ positions }) {
  const [result, reload, error] = useLoad("/players?per_page=100", { items: [] });
  const blank = { name: "", email: "", phone: "", primary_position_id: "", secondary_position_id: "", ...initialSkills };
...
function App() {
  ...
  const [bootstrap, reload, loadError] = useLoad("/public/bootstrap", { events: [], positions: [], players: { items: [] }, settings: {} });
  const token = useMemo(() => location.pathname.match(/^\/confirmar\/(.+)$/)?.[1], []);
  if (token) return <Confirmation token={token} goHome={() => { history.replaceState({}, "", "/"); location.reload(); }} />;
  return <><header className="topbar"><button className="brand" onClick={() => setPage("inscricao")}><img src="/VoleiFlow_logo.svg" alt="" /><span>Volei<b>Flow</b></span></button><nav><button className={page === "inscricao" ? "active" : ""} onClick={() => setPage("inscricao")}>Inscrição</button><button className={page === "situacao" ? "active" : ""} onClick={() => setPage("situacao")}>Meus times</button><button className={page === "admin" ? "active" : ""} onClick={() => setPage("admin")}>Admin</button></nav><Connectivity /></header><main><Notice tone="error">{loadError}</Notice>{page === "inscricao" && <Signup bootstrap={bootstrap} reload={reload} />}{page === "situacao" && <Situation bootstrap={bootstrap} />}{page === "admin" && <AdminPortal bootstrap={bootstrap} reloadBootstrap={reload} />}</main><footer><span>VoleiFlow · jogo organizado, quadra equilibrada.</span><span>Feito para funcionar até com sinal ruim.</span></footer></>;
}
